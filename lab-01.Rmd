...
##attraction, admission, region,setting: Character
##n_2021,n_2022: Integer
...
##Free
#How many Outside attractions are there in the "Yorkshire and the Humber" region that gives "Members" free admission, which had more than 100,000 visitors in 2022?
##3
```
...
#Note: n_2021 contains empty cells, so na.rm must be set to true such that these null values are not considered
```
...
**NA stands for Non-Applicable, and it is returned as a result because n_2021 contains empty cells**
...
**As per the mean help page the argument na.rm is "a logical evaluating to TRUE or FALSE indicating whether NA values should be stripped before the computation proceeds."**
...
**Mean > median implies most of the data lies to the right of the mean and the data is therefore negatively skewed, or skewed to the left.**
...
visitors_with_nations %>% count(nation)
```
